<template>
    <div class="vue-contribution">
        <svg :width="mW" :height="mH" class="svg">
                <text :style="{fontSize: fontSize + 'px'}"  :fill="fontColor" :x="textX(i)" :y="textY(i)" v-for="(m, i) in month" :key="m.id">{{m}}</text>
                <g v-for="(val, key, index) in mdata" :key="key.id" >
                    <rect
                        class="rect"
                        :width="rectWidth"
                        :height="rectHeight"
